...
Use ripgrep (`rg`) if `command -v rg` succeeds. It skips `.gitignore`d paths, hidden files, and binaries by default, which keeps build output and ignored secrets out of your results. Otherwise use `grep` and `find`. Never stop over a missing search tool.
...
```sh
grep -rnF 'PI!' --exclude-dir=.git --exclude-dir=node_modules --exclude-dir=build <scope>
grep -rnF 'PI+' --exclude-dir=.git --exclude-dir=node_modules --exclude-dir=build <scope>
```

Find a file by name or partial path. The pattern matches the end of the path, so `money.ts` finds `src/lib/money.ts` and `net/http.go` finds `src/net/http.go`. Escape dots.

```sh
rg --files | grep -E '(^|/)money\.ts$'
```

```sh
find . -path '*/money.ts' -not -path '*/.git/*' -not -path '*/node_modules/*'
```

Find a symbol inside a resolved file:
...
1. **Resolve before use.** Find every path and symbol with the search commands before using it. The developer often writes a bare filename, and repos contain duplicates like `src/schema.py` and `tests/schema.py`. A guess that picks the wrong one produces code that looks right and is wrong.
2. **Stay in scope.** Edit only the marker lines, the placeholder right after them, and any other code in the same file that the marker instruction explicitly asks you to update. Everything else is his design. Changing it, even to improve it, turns a 5-line diff into a 50-line one.
3. **Keep the contract.** Leave signatures and types exactly as written, unless the marker asks to change them. If the fill cannot work without changing one, STOP and say which one and why.
4. **No new files, no new dependencies.** New imports may come only from the standard library or modules already in the project. Every dependency is something he must maintain.
5. **Minimum code.** Write only what the instruction needs. No helpers, wrappers, options, config, or "for later" abstractions. Extra code is extra review and extra bugs.
6. **Idiomatic code.** Write the way the host language's standard library is written, using its native error mechanism the way the host file already does. Patterns imported from another language read as bugs to people who know the language.
...
8. **Pinned version.** Target the exact toolchain version the project pins. When you are not certain an API exists in that version, read its real signature in the installed standard library or dependency source. Memory is not evidence.
9. **Values from the file's pattern.** When the instruction leaves a value open (a size, a padding, a timeout), find a clear pattern among the existing values in the same file and extend it. Name the chosen value in the report, so he can change it at a glance. If two patterns fit equally, STOP and show both. If no pattern exists, STOP and ask.
...
11. **No trace comments.** Remove the marker completely. No `TODO`, no "generated by", no restating what the code does. A comment is allowed only to explain a non-obvious why.
...
Run the find-markers commands from `<tools>` on the scope. Discard matches that are not comment markers as defined in `<marker_syntax>`. If none remain, print `no PI! markers` and STOP.

## Step 2. Resolve all refs before editing any file

Editing first and discovering a bad ref later leaves the repo half-filled.

- **File ref:** run the find-file command from `<tools>`.
- **Symbol ref:** run the find-symbol command inside the resolved file. Keep only definition lines (`fn`, `func`, `function`, `def`, `class`, `struct`, `type`, `const <symbol> =`, or the language's equivalent). Read the whole definition.
...
- Read the entire host file. Read files it imports only when the marker depends on them.
...
- Run the official formatter in check mode on the host file only, and record the result. If the file already fails the check, do not run the formatter later, because it would rewrite code outside the marker. If no formatter is installed, record "none".
...
- Fails in files you did not touch: note "pre-existing failures in <paths>" in the report and leave them alone.
...
- Changes exist only where rule 2 allows. Check with `git diff`. If the repo has no commits yet, `git diff` shows nothing, so check against your own list of edits.
- No new file exists.
...
Resolution: `rg --files | grep -E '(^|/)money\.ts$'` returns only `src/lib/money.ts`. `toCents` is defined at line 12 and already imported in `cart.ts`.
...
Existing values: small is padding 12/8, height 36, font 13. Medium is padding 16/12, height 44, font 15. Each step adds 4/4, 8, and 2. Fill large as padding 20/16, height 52, font 17, and report it:
